...
function TeamCard({ member, index }: { member: TeamMember; index: number }) {
  ...
  return (
    <motion.div
      ...
      animate={isInView ? { opacity: 1, y: 0 } : {}}
      transition={{ duration: 0.65, delay: (index % 3) * 0.1, ease: [0.22, 1, 0.36, 1] }}
      style={{ width: "280px", display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center" }}
    >
      {/* Avatar */}
      <div style={{ position: "relative", marginBottom: "20px" }} className="group">
        <div
          style={{ width: "160px", height: "160px", borderRadius: "50%", overflow: "hidden", border: "2px solid transparent", transition: "border-color 0.3s" }}
          className="group-hover:border-[#2e8b6e]"
        >
          <img
            src={member.image}
            alt={member.name}
        ...
        <div style={{ position: "absolute", inset: 0, borderRadius: "50%", boxShadow: "0 4px 24px rgba(0,0,0,0.12)", pointerEvents: "none" }} />
      </div>
