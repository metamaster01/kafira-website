...
// //             >
// //               {INTL_TRIPS.map(trip => (
// //                 <div key={trip.id} className="intl-card" style={{ scrollSnapAlign: 'start'}}>
...
//   // heading scroll reveal — fromTo avoids the set+to race condition
//   useEffect(() => {
//     const el = headRef.current; if (!el) return;
//     gsap.fromTo(
//       el,
...
//   // cards stagger reveal — use section as trigger (more reliable on mobile)
//   // and a very generous start so it fires even if layout is slightly off
//   useEffect(() => {
//     const track = trackRef.current; if (!track) return;
//     const cards = track.querySelectorAll<HTMLElement>('.intl-card');
//     gsap.fromTo(
...
//         scrollTrigger: { trigger: track, start: 'top 100%', once: true } }
//     );
//   }, []);
