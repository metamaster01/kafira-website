...
// ─────────────────────────────────────────────────────────────────
const PLACES = [
  { id: 1, name: 'Leh Ladakh',        tag: 'High Altitude',    emoji: '🏔' },
  { id: 2, name: 'Shimla hills', tag: 'Serene Waters',    emoji: '🌊' },
  { id: 3, name: 'Maharastra Forts',   tag: 'Royal Heritage',   emoji: '🏰' },
  { id: 4, name: 'Manali Valleys',    tag: 'Alpine Escape',    emoji: '🌲' },
  { id: 5, name: 'Goa Beaches',       tag: 'Sun & Sand',       emoji: '🏖' },
  { id: 6, name: 'Maldives inland',       tag: 'Misty Highlands',  emoji: '☁️' },
];
...
// ─────────────────────────────────────────────────────────────────
function VideoStrip({ videos, reverse }: { videos: number[]; reverse: boolean }) {
  ...
  return (
    <div
      style={{ overflow: 'hidden', height: '100%', position: 'relative', borderRadius: 20 }}
      onMouseEnter={pause}
      onMouseLeave={resume}
    >
      {/* top/bottom fade masks */}
      <div style={{ position:'absolute',top:0,left:0,right:0,height:80,background:'linear-gradient(to bottom,#f5f0e8,transparent)',zIndex:10,pointerEvents:'none' }}/>
      <div style={{ position:'absolute',bottom:0,left:0,right:0,height:80,background:'linear-gradient(to top,#f5f0e8,transparent)',zIndex:10,pointerEvents:'none' }}/>

      <div ref={trackRef} style={{ display:'flex', flexDirection:'column', gap:12 }}>
        {videos.map((n, i) => (
          <div key={i} style={{
            flexShrink: 0,
            height: 220,
  ...
  return (
    ...
    >
      ...
      <div style={{ position: 'absolute', bottom: 12, left: 14, right: 14 }}>
        ...
        <div style={{
          height: 2, width: 28, borderRadius: 1,
      ...
      <div style={{
        ...
        <div style={{
          ...
          color: '#fff', fontFamily: 'Outfit,sans-serif', fontSize: 12, fontWeight: 700,
...
function SectionHeading() {
  ...
  return (
    <div ref={ref} style={{ textAlign: 'center', maxWidth: 600, margin: '0 auto 52px' }}>
      ...
      <h2 style={{
        ...
        Every destination,
        <br/>
        <span style={{ fontStyle: 'italic', color: '#c9a84c' }}>a world of its own</span>
      </h2>

      <p style={{
        fontFamily: 'Outfit,sans-serif', fontSize: 16,
        color: '#6b5e4e', lineHeight: 1.7,
      }}>
        From snow-capped Himalayan passes to sun-drenched Goan shores —
        Kafira takes you exactly where your heart calls.
      </p>
    </div>
  );
}
    ...
    <div ref={ref} style={{
      ...
      {STATS.map((s, i) => (
        ...
        >
          <div style={{
            fontFamily: 'Playfair Display,serif', fontWeight: 800,
            fontSize: 'clamp(22px,2.5vw,32px)', color: '#c9a84c', lineHeight: 1,
          }}>{s.val}</div>
          <div style={{
            fontFamily: 'Outfit,sans-serif', fontSize: 12,
            color: '#6b5e4e', marginTop: 6, letterSpacing: '0.04em',
          }}>{s.label}</div>
        </div>
      ))}
    </div>
...
export default function ExploreSection() {
  ...
  return (
    <section
      id="explore"
      style={{ background: '#f5f0e8', padding: '96px 0 80px', overflow: 'hidden' }}
    >
