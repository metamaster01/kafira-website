...
// ─────────────────────────────────────────────────────
//  AllDestinations.tsx
//  Circular avatar grid — India-first, 3-row paginated
// ─────────────────────────────────────────────────────
...
// ── Sort: India first (preserving file order within each region) ──
// Uses a stable sort — items within the same region keep their original index order.
const ALL = [...DESTINATIONS].sort((a, b) => {
  if (a.region === b.region) return 0;
  return a.region === 'india' ? -1 : 1;
});

// ── Responsive columns per row ─────────────────────────
// Used to keep exactly 3 rows per page.
function useColumnsPerRow(): number {
  // Read once on mount; re-reads on resize via the parent's resize handler.
...
export default function AllDestinations() {
  ...
  // Recompute columns on every render (window resize triggers re-render via state below)
  const [, forceUpdate] = useState(0);
  const cols = useColumnsPerRow();

  // Keep cols stable via resize listener
  useState(() => {
  ...
  // ── Filtered list (India first, then international) ──
  const filtered = useMemo(() => {
    if (filter === 'all') return ALL;
    return ALL.filter(d => d.region === filter);
  }, [filter]);

  // ── Pagination ───────────────────────────────────────
  // Exactly 3 rows per page; cols = number of avatars per row.
  const ROWS_PER_PAGE = 3;
  const perPage = cols * ROWS_PER_PAGE;
  const totalPages = Math.max(1, Math.ceil(filtered.length / perPage));

  // Clamp page when filter/resize shrinks totalPages
  const safePage = Math.min(page, totalPages - 1);
  const pageSlice = filtered.slice(safePage * perPage, (safePage + 1) * perPage);

  const handleFilter = useCallback((f: typeof filter) => {
    setFilter(f);
    setPage(0);
  }, []);
  ...
  const handlePage = useCallback((p: number) => {
    setPage(p);
    // Scroll section top into view on mobile
  ...
  return (
    ...
    >
      <div style={{ maxWidth: 1160, margin: '0 auto', padding: '0 clamp(20px,5vw,56px)' }}>
        ...
        {/* ── avatar grid (exactly 3 rows per page) ── */}
        <motion.div
          layout
          style={{
            display: 'flex',
